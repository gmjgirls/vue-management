...
router.post('/goodsadd', (req, res) => {
	...
	// res.send('哈哈哈')
	// 接收参数
	let {classify, barCode, goodsName,goodsinprice,goodsprice,goodsNum,member} = req.body;

	// // 构造sql
	const sqlStr = `insert into goods(classify, barCode, goodsName, goodsinprice, goodsprice, goodsNum, member) values('${classify}','${barCode}','${goodsName}','${goodsinprice}','${goodsprice}','${goodsNum}','${member}')` 
...
/* 保存修改请求 */ 
router.post('/seveedit', (req, res) => {

	// 接收新数据 和 老id
	let {classify, goodsName, id} = req.body;

	// 准备sql
	const sqlStr = `update goods set classify='${classify}', goodsName='${goodsName}' where id=${id}`;
	// 执行sql
	connection.query(sqlStr, (err, data) => {
		if (err) throw err;
		// 判断
